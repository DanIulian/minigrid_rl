...
class BaseAlgo(ABC):
    ...
    def _initialize_log_values(self, log_metrics_names):
    ...
    def update_experience_values(self, curr_obs, curr_act, curr_reward,
        ...
        if self.reshape_reward is not None:
            self.rewards_ext[curr_step] = torch.tensor([
                ...
                for obs_, action_, reward_, done_ in zip(curr_obs, curr_act, curr_reward, curr_done)
            ], device=self.device)
        else:
            self.rewards_ext[curr_step] = torch.tensor(curr_reward, device=self.device)
    ...
    def compute_advantages(self, next_value):
        ...
        for i in reversed(range(self.num_frames_per_proc)):
            ...
            delta = self.rewards_ext[i] + self.discount * next_value * next_mask - self.values_ext[i]
