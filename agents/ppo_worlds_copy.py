...
class PPOWorlds(TwoValueHeadsBaseGeneral):
    ...
    def update_parameters(self):
        ...
        if self.pre_fill_memories:
            ...
            for i in range(num_frames_per_proc)[::-1]:
                ...
                with torch.no_grad():
                    ...
                    #                     f.actions_onehot[i])
                    _, f.agworld_mems[i], f.agworld_embs[i] = \
                        agworld_network(obs, agworld_mem * masks, prev_actions,
                                        f.actions_onehot[i])
        ...
        for inds in self._get_batches_starting_indexes(recurrence=recurrence_worlds, padding=1):
            ...
            for i in range(recurrence_worlds)[::-1]:
                ...
                crt_actions = f.actions_onehot[inds + i - 1].detach()
            ...
            for i in range(recurrence_worlds-1)[::-1]:
                ...
                next_mask = f.mask[inds + i].long().detach()
                next_mask = next_mask.squeeze(1).type(torch.ByteTensor)
    ...
    def evaluate(self):
        ...
        eval_ag_memory_idx = columns.index("eval_ag_memory")
        new_agworld_emb_idx = columns.index("new_agworld_emb")
        ...
        for i in range(steps-1)[::-1]:
            ...
            with torch.no_grad():
                ...
                if prev_agworld_emb is not None:
                    pred_act = agworld_network.forward_action(eval_ag_memory, prev_agworld_emb)

                prev_agworld_emb = new_agworld_emb

            transitions[i][eval_ag_memory_idx] = eval_ag_memory.cpu()
            transitions[i][new_agworld_emb_idx] = new_agworld_emb.cpu()
            transitions[i+1][pred_act_idx] = pred_act.cpu()
