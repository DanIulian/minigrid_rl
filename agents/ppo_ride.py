...
class PPORide(TwoValueHeadsBaseGeneral):
    ...
    def calculate_intrinsic_reward(self, exps: DictList, dst_intrinsic_r: torch.Tensor):
        ...
        states_visitation = torch.ones(num_frames_per_proc + 1, num_procs, device=device)
        states_visitation[:-1] = f.states_visitation
        ...
        dst_intrinsic_r = (embedding_states[1:] - embedding_states[:-1]).detach().norm(dim=2)
        dst_intrinsic_r = dst_intrinsic_r.div_(states_visitation[1:])
