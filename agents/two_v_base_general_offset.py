...
class TwoValueHeadsBaseGeneralOffset(ABC):
    ...
    def __init__(self, envs, acmodel, num_frames_per_proc, discount, gae_lambda, entropy_coef,
        ...
        self.acmodel.train()
        self.train_frames_per_proc = num_frames_per_proc
        self.num_frames_per_proc = num_frames_per_proc + offset_exp
    ...
    def collect_experiences(self):
        ...
        """
        start_frame = experience_offset = self.offset_exp

        if self._first_experience:
            start_frame = 0
            self._first_experience = False
        else:
            for ref in [self.obss, self.memories, self.masks, self.actions, self.values_ext,
                        self.values_int, self.log_probs]:
                ref[:experience_offset] = ref[-experience_offset:]

        for i in range(start_frame, self.num_frames_per_proc):
        ...
        self.rewards_int = self.rewards_int.transpose(0, 1)
        self.rewards_int = self.calculate_intrinsic_reward(exps, self.rewards_int).transpose(0, 1)
